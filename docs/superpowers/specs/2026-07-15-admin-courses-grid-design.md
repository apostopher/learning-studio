...
keys.adminCourses(), queryFn: () => listAdminCoursesFn(), staleTime: 60_000 })`.
...
- Create: `src/routes/_authed/admin.tsx`, `src/db/admin.ts`,
  `src/lib/admin-functions.ts`, `src/hooks/data/use-admin-courses.ts`,
...
- Modify: `src/hooks/data/keys.ts`, `package.json` (script), `src/routeTree.gen.ts`
  (regenerated by the router plugin — not hand-edited).
