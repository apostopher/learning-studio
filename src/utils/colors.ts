...
/** Coerce Color coords to non-null numbers (null → 0 for powerless channels like achromatic hue) */
function getCoords(color: Color): [number, number, number] {
  return color.coords.map((v) => v ?? 0) as [number, number, number];
}
...
export const generateRadixColors = ({
  ...
  accentScaleColors[10].coords[1] = Math.min(
    Math.max(
      getCoords(accentScaleColors[8])[1],
      getCoords(accentScaleColors[7])[1],
    ),
    getCoords(accentScaleColors[10])[1],
  );
  accentScaleColors[11].coords[1] = Math.min(
    ...
    getCoords(accentScaleColors[11])[1],
  );
...
function getScaleFromColor(
  ...
  const ratioC = getCoords(source)[1] / getCoords(baseColor)[1];

  // Modify hue and chroma of the scale to match the source color
  scale.forEach((color) => {
    color.coords[1] = Math.min(
      getCoords(source)[1] * 1.5,
      getCoords(color)[1] * ratioC,
    );
    color.coords[2] = getCoords(source)[2];
  });

  // Light mode
  if (getCoords(scale[0])[0] > 0.5) {
    const lightnessScale = scale.map((c) => getCoords(c)[0]);
    const backgroundL = Math.max(0, Math.min(1, getCoords(backgroundColor)[0]));
  ...
  const referenceBackgroundColorL = getCoords(scale[0])[0];
  const backgroundColorL = Math.max(
    0,
    Math.min(1, getCoords(backgroundColor)[0]),
  );
  ...
  const lightnessScale = scale.map((c) => getCoords(c)[0]);
  const backgroundL = getCoords(backgroundColor)[0];
  const newLightnessScale = transposeProgressionStart(
    backgroundL,
    lightnessScale,
    ease,
...
function getTextColor(background: Color) {
  ...
  if (Math.abs(white.contrastAPCA(background)) < 40) {
    const [, C, H] = getCoords(background);
...
function getAlphaColorSrgb(
  ...
  const [r, g, b, a] = getAlphaColor(
    getCoords(new Color(targetColor).to("srgb")),
    getCoords(new Color(backgroundColor).to("srgb")),
    255,
    255,
    targetAlpha,
  );
...
function getAlphaColorP3(
  ...
  const [r, g, b, a] = getAlphaColor(
    getCoords(new Color(targetColor).to("p3")),
    getCoords(new Color(backgroundColor).to("p3")),
...
function toOklchString(color: Color) {
  const L = +(getCoords(color)[0] * 100).toFixed(1);
  return color
    .to("oklch")
    .toString({ precision: 4 })
    .replace(/(\S+)(.+)/, `oklch(${L}%$2`);
