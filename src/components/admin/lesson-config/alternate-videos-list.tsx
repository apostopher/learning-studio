    ...
    <ul className="flex flex-col divide-y divide-gray-6 rounded-lg border border-gray-6">
      {rows.map((row) => (
        <li key={row.lang} className="flex items-center gap-3 px-3.5 py-2.5">
          {/* Decorative for AT: the label beside it already names the language. */}
          <span
            aria-hidden="true"
            className="rounded bg-gray-3 px-1.5 py-0.5 font-medium text-secondary text-xs uppercase"
          >
            {row.lang}
          </span>
