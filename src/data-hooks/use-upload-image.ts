...
/**
 * Optimize a source image (already cropped to 16:9) to AVIF + WebP in the
 * browser and upload both directly to Vercel Blob via the admin token endpoint,
 * returning their public URLs. `pathPrefix` namespaces the blob (e.g.
 * "courses", "modules"). The server never receives the bytes — `upload()` gets
 * a short-lived client token from /api/admin/uploads and streams to Blob
 * storage directly.
 */
export function useUploadImage(pathPrefix: string) {
  return useMutation<UploadedImage, Error, Blob>({
    mutationFn: async (source) => {
      const { avif, webp } = await optimizeImage(source);
